...
let subset name file (index : StreamWriter) selector =
    let sub =
        ...
        |> Seq.map (fun (scramble, rendered) -> sprintf "1. `%s` (%s)" rendered (Render.stepsToString scramble))
        |> List.ofSeq
...
let writeCase name file alg = case alg |> subset name file index
...
writeCase "H-Column" "h_column" "R U R' U R U' R' U R U2 R'"
