...
let subset name file (index : StreamWriter) selector =
    ...
    File.WriteAllLines($"{file}.md", contents)
    index.WriteLine($"- [{name}]({file}.md)")
...
let fourUpBest cube =
    fourUp cube &&
    ((lrEdge Face.U Sticker.U cube && lrEdge Face.U Sticker.D cube) ||
    (lrEdge Face.U Sticker.L cube && lrEdge Face.U Sticker.R cube))

let fourUpGood cube =
    fourUp cube &&
    ((lrEdge Face.U Sticker.U cube || lrEdge Face.U Sticker.D cube || lrEdge Face.U Sticker.L cube || lrEdge Face.U Sticker.R cube) &&
    (lrEdge Face.F Sticker.D cube || lrEdge Face.B Sticker.U cube))

let fourUpBottom cube =
    fourUp cube &&
    lrEdge Face.F Sticker.D cube && lrEdge Face.B Sticker.U cube

let fourUpWorst cube =
    fourUp cube &&
    (lrEdge Face.U Sticker.L cube || lrEdge Face.U Sticker.R cube) &&
    (lrEdge Face.U Sticker.U cube || lrEdge Face.U Sticker.D cube)

let index = new StreamWriter(File.OpenWrite "EOLR.md")
index.WriteLine("# LSE EOLR/FB ([generated by](./LSE.fs))")
index.WriteLine()
index.WriteLine("See [Jeremy's excellent \"EOLR Described\"](https://docs.google.com/document/d/1rb5M9_5CTlozLu9acFIgqq9LYYKaUdnMDX0vcLOURT4/edit?usp=sharing)")
index.WriteLine();
...
index.WriteLine()
index.WriteLine("## EOLR 4-0 Cases")
index.WriteLine()
subset "EOLR Best 4-0" "eolr_4-0_best" index fourUpBest
subset "EOLR Good 4-0" "eolr_4-0_good" index fourUpGood
subset "EOLR Bottom 4-0" "eolr_4-0_bottom" index fourUpBottom
subset "EOLR Worst 4-0" "eolr_4-0_worst" index fourUpWorst
